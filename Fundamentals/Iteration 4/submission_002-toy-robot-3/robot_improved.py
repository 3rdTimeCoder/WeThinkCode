...
def replay(name, com, history, deg_ind, deg, pos):
    ...
    digits = get_digits(com)
    limit = get_range(com, history_len, digits)
    history_replayable = check_history_replayable(digits, history_len)
...
def get_digits(com):
    """parses through command and returns the digits part of it"""
...
def check_history_replayable(digits, history_len):
    """Checks if the args to see if history can be replayed"""
    
    if len(digits) == 1 and digits[0] <= history_len: 
        history_replayable = True
    elif len(digits) == 2 and digits[0] > digits[1]: 
        history_replayable = True
    else: 
        history_replayable = False
        
    return history_replayable


def get_range(com, history_len, digits):
    """Gets the range used to loop through history"""
    
    limit = range(history_len)

    if len(digits) == 1: 
        limit = range(-1*digits[0], 0, 1)
    elif len(digits) == 2: 
        limit = range(-1*digits[0], -1*digits[1])
...
def move_robot(name, com, steps, pos, deg, silent=False):
    ...
    if pos_changed and not silent: 
        print(return_action(com, steps, name))
...
def robot_start():
    ...
    while True:
        ...
        if com == 'off': 
            print(f"{name}: {return_action(com)}"); break
        elif 'replay' in com: 
